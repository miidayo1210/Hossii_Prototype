...
> **分類:** `[Core]` Supabaseテーブル構成・RLS・Storage・Realtime
> **関連:** [core-concepts.md](../../core/core-concepts.md) §4（共通データ構造）

> 最終更新: 2026-04-12（投稿同期・フリー永続化の課題整理を追記）
...
| 投稿画面の吹き出し色（下書き） | 最後に選んだテーマ ID とスウォッチ色 | localStorage キー `hossii.lastPostBubbleColorPrefs`（[54](./54_吹き出しの色カラーパレット%20.md)） | 低 |
| Listen 設定 | Listen モード、音声ログ設定、感情ログ設定 | localStorage（移行しない） | 低 |
| フィルター状態 | 投稿フィルターの選択状態 | localStorage（移行しない） | 低 |
| 投稿（Supabase 未設定時のフォールバック） | 同一端末内の投稿一覧のスナップショット（吹き出し・**フリー投稿**を含む） | localStorage キー **`hossii.hossiis`**（`src/core/utils/storage.ts` の `saveHossiis`／`loadHossiis`） | 中※ |

> **性能最適化（✅ 実装済み）:** Supabase 有効時は reducer から `saveHossiis` 全件 serialize を **停止**（`hossiiPersistence.ts`）。投稿は `entitiesById` + `orderedIdsByQueryKey` で正規化（[87](./87_スペース表示パフォーマンス最適化.md) §5）。

※ **Supabase が有効な環境では**、投稿の初期状態は DB 取得が正とし、`hossii.hossiis` は **リロード時の初期表示には使わない**（フラッシュ防止のため）。未設定ビルド・ローカルデモでは当キーが投稿の永続化に使われる。
...
**キャッシュと Supabase の整合:** slug 直アクセス時、localStorage の古いスペース情報で `background` 等が上書きされないよう、サーバーからの再取得・マージを行う（[82_スペース情報キャッシュ再同期](./82_スペース情報キャッシュ再同期.md)）。
...
**フリー投稿（`post_kind` / `postKind: canvas`）**: 完成ラスタの URL を `image_url`（DB）／`imageUrl`（クライアント型）に保存する。**Supabase 未設定**時は Storage に上げられないため **data URL** になり得る。

#### localStorage の `hossii.hossiis`（デモ・オフライン）

- **目的**: `isSupabaseConfigured === false` のとき、投稿を端末内でリロード後も復元する。
- **Supabase 有効時**: reducer 更新のたびに **全件 `saveHossiis` しない**（[87](./87_スペース表示パフォーマンス最適化.md)）。デモ fallback 用に最大 **200 件** cap。
- **実装**: `saveHossiis` は `Hossii` を JSON 化して保存する。`normalizeHossii`（`HossiiStoreProvider`）が復元時に読むフィールドと **揃える**（`imageUrl`、`postKind`、座標・スケール・`bubbleColor`・タグ類など）。**一部フィールドだけ保存**していた場合、フリー投稿は **`imageUrl` が欠落**し、ツリー上で canvas として表示できない／表示モード「画像」では **一覧から除外**され、利用者には「消えた」ように見える。
- **注意**: data URL は JSON と合わせて **数 MB** に達しうる。ブラウザの **localStorage 上限**（目安 5MB 前後）を超えると `setItem` が失敗し、**保存されない**（実装は `try/catch` で握りつぶす）。容量超過時はコンソールに **`[storage] saveHossiis: localStorage quota exceeded`** を出す。本番運用では **Supabase ＋ Storage** を前提とする。

---

### 3.1 課題：投稿が「消える」見え方（フリー・同期まわり）

フリー投稿や画像付き投稿が **リロード後に無い**／**操作中に一瞬消える**と報告される場合、次を **課題カタログ**として切り分ける（詳細・表形式は [69_投稿のUI設定.md](./69_投稿のUI設定.md) の **「課題整理：フリー投稿が消える」**）。

| 区分 | 概要 |
|------|------|
| **端末ローカル** | `saveHossiis` のフィールド不足（**対応済**）／**localStorage 容量**（data URL 肥大）。 |
| **クライアント同期** | スペース一覧の更新のたびに投稿一覧を空にして再 fetch していた（**`spaceIdsSignature` で緩和済**）。 |
| **Realtime と楽観更新** | 自己 INSERT を無条件スキップし、楽観行が先に消えていると **画面に復帰しない**（**state に無ければ DB 行から復元するよう対応済**）。 |
| **サーバ** | `insertHossii` 失敗（RLS・スキーマ・`post_kind` 未マイグレ等）。**失敗時は楽観行を削除**するよう対応済だが、**ユーザー向けエラー表示は未**（コンソール `[hossiisApi] insertHossii error`）。 |
...
| space_settings | スペース UI・機能設定 | ✅ マイグレーション済み・読み書き実装済み（`spaceSettingsApi.ts`） |
| stamps | スタンプカードデータ | ✅ マイグレーション済み・読み書き実装済み（`stampsApi.ts`、ログイン済みのみ） |
| communities | コミュニティ情報 | ✅ マイグレーション済み・読み書き実装済み |
| hossii_likes | いいねデータ | ✅ マイグレーション済み・読み書き実装済み（`likesApi.ts`） |
...
> **フリー投稿（canvas）:** Supabase 有効時は `uploadCanvasPostImage`（`imageStorageApi.ts`）で `hossii-images/canvas/{spaceId}/{hossiiId}.png|webp` に保存し、公開 URL を `image_url` に書く。アップロード失敗時はエラー表示し、行 INSERT を避ける。**Supabase 未設定時**は完成 PNG を **data URL** のまま `imageUrl` に載せ、`hossii.hossiis` で永続化する（上記「localStorage」節・容量上限に注意）。
...
| Realtime 購読（hossiis INSERT/DELETE） | ✅ 完了（`HossiiStoreProvider.tsx` 内） |
...
| space_settings Supabase 読み書き | ✅ 完了（`src/core/utils/spaceSettingsApi.ts`）Supabase 優先・localStorage フォールバック |
| stamps Supabase 読み書き | ✅ 完了（`src/core/utils/stampsApi.ts`）ログイン済みユーザーのみ Supabase に保存 |
| hossii_likes テーブル・API | ✅ 完了（`src/core/utils/likesApi.ts`・`20260306000000_add_likes.sql`） |
| 投稿の localStorage 永続化（Supabase 未設定） | ✅ 完了（`saveHossiis` が `imageUrl`・`postKind`・座標等を保存。フリー投稿のリロード復元に必要） |
| 投稿 store 正規化・Supabase 時 localStorage 全件保存停止 | ✅ 完了（[87](./87_スペース表示パフォーマンス最適化.md) — `hossiiEntitiesState.ts` / `hossiiPersistence.ts`） |
| 投稿一覧の再フェッチ条件の整理 | ✅ 完了（`spaceIdsSignature`／Realtime 自己 INSERT 時の復元／`insertHossii` 失敗時の楽観ロールバック。詳細は **§3.1**・[69](./69_投稿のUI設定.md)） |
| テーブル RLS 本番設定 | **未実装** |
...
| 日付 | 内容 |
|------|------|
| 2026-04-12 | **§3.1 課題：投稿が消える見え方** を追加。69 と相互参照。実装ステータスに同期整理の行を追加。Realtime の記載ファイル名を `HossiiStoreProvider.tsx` に修正。 |
| 2026-06-23 | [87](./87_スペース表示パフォーマンス最適化.md) 実装反映: Supabase 時 `saveHossiis` 停止・正規化 store・実装ステータス行追加 |
