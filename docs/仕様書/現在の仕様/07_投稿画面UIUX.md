...
**概要**: 投稿時に吹き出しの色をテーマ別プリセットから選択できる。詳細は [54_吹き出しの色カラーパレット](./54_吹き出しの色カラーパレット%20.md)。

**UI**: 4 テーマ × 各 8 色に加え未指定のデフォルト。初回の既定テーマはビビッド。ビビッドの 8 色の一例:
`#FF6B6B` / `#4ECDC4` / `#45B7D1` / `#96CEB4` / `#FFEAA7` / `#DDA0DD` / `#98D8C8` / `#F7DC6F`

**仕様**:
- 未選択時はデフォルト色（グラデーション）
- 選択色は投稿フォームのプレビューには反映しない（スペース画面のバブルに反映）
- **送信成功後もテーマと色の選択はクリアしない**。次回オープン時の初期値用に、端末ローカルへテーマ ID と色を保存する（キーは 54 参照）

**データ（サーバー）**: `hossiis.bubble_color` varchar(7)（例: `"#FF6B6B"`）。テーマ ID は DB に持たない。
...
4. 成功トースト表示
5. 連続投稿が OFF のとき、800ms 後にスペース画面（Home）へ遷移（ON のときは投稿画面に留まる。パネルモードは [51_簡単投稿](./51_簡単投稿.md)）
6. フォームをリセット（選択感情・テキスト・タグ・画像など。**吹き出しのテーマ／色はリセットしない** — 上記 F01）
