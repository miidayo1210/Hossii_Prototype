...
- 表示期間フィルタ: 1日/1週/1月/全期間（LeftControlBar、**デフォルト全期間**）
- 表示モード: フル / バブル / 画像のみ の3種類（LeftControlBar、**デフォルトフル表示**）
- 並びモード: ランダム / 投稿順 / **投稿者まとめ**（LeftControlBar、**デフォルトランダム** — [81](./81_投稿者別まとめ表示モード.md)）
- **PC 表示切替:** **星モード**（既定）↔ **カスタムモード**（吹き出し配置）。右上 pill は現在モードの反対側ラベル（星 ON 時「カスタム」、カスタム時「☆ 星」）— [84](./84_スペース表示モード拡張.md)
...
- **スペース説明:** `Space.description`（最大 50 文字）。PC は TopBar スペース名右隣 pill、モバイルは左上タイトル pill 右隣（[91](./91_スペース説明表示.md)）
- **吹き出し全文:** 省略されている長文はホバー（星モード / カスタムモード）またはリサイズ前展開で全文可（[99](./99_吹き出し全文表示.md) — 📋 未実装）
...
**長文とリサイズ（[99_吹き出し全文表示](./99_吹き出し全文表示.md) — 📋 未実装）:**

省略されている本文がある吹き出しを拡大する場合、**スケールを上げる前に** 切り詰めを解除して隠れ行を表示する（Phase A → Phase B）。詳細は 99 §4.3。
