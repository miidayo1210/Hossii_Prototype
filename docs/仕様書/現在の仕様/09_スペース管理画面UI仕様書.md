...
> **分類:** `[Implementation]` スペース設定画面・管理タブのUI詳細
> **関連:** [feature-flags.md](../../feature-flags.md)
...
| タブ ID | タブ名 | 内容 | 仕様書 |
|---------|--------|------|--------|
| `general` | 一般 | スペース名・基本設定 | — |
| `postForm` | 投稿フォーム | 投稿パネルの表示項目・必須制御 | [86](./86_投稿フォーム項目設定.md) |
| `hossii` | Hossii カスタマイズ | キャラ画像・色 | 本書 §A01 |
| `background` | 背景 | 背景画像・カラー設定 | [24](./24_スペース管理_各スペースの設定_背景設定.md) |
| `share` | 共有 | 招待 URL・QR コード | — |
| `moderation` | モデレーション | 投稿管理・非表示 | [28](./28_スペース投稿非表示.md) |
| `decoration` | 装飾 | 吹き出し装飾設定 | — |
| `featureFlags` | Feature Flags | 機能フラグ ON/OFF | [50](./50_Feature%20frag%20整理.md) |
| `tags` | タグ | プリセットタグ管理 | [26](./26_タグとフィルタリング機能.md) |
| `neighbors` | 隣人 | 隣接スペース設定 | [44](./44_隣人システム.md) |

認証・コミュニティ登録・ログインフローは **`05_管理者ログイン画面.md`** を参照。

### 投稿フォームタブ（`PostFormTab`）

> **実装ステータス**: ✅ 実装済み（`PostFormTab.tsx`）。詳細は [86_投稿フォーム項目設定](./86_投稿フォーム項目設定.md)。

- `general` タブの直後に配置（全スペース管理者向け）
- 7 項目（メッセージ・気持ち・タグ・写真・吹き出し色・吹き出し形状・数値入力）の「表示」「入力必須」トグル
- トグル変更は即時保存（localStorage + Supabase upsert）
- 表示 OFF 時は必須トグル無効化。表示 OFF にした際、必須 ON だった場合は自動リセット
- 全項目表示 OFF 時に警告文を表示
- `bubble_shapes_extended` が OFF のとき吹き出し形状行に Feature Flag バッジ
...
| スペース設定画面（詳細編集）へのリンク | ✅ 実装済み（⚙️ 設定ボタン） |
| 共有スペースと個人スペースの一覧分離 | **未実装** → [113](./docs/113_管理者スペース一覧の共有と個人の分離.md) |
| 個人スペーステンプレートの配置（個人セクション内） | **未実装** → 同上 |
...
> **✅ 実装済み** — `src/core/utils/moderationLogsApi.ts` + `supabase/migrations/20260228000000_add_moderation_logs.sql`
...
| ステータス | 機能 ID | 内容 |
|---|---|---|
| ✅ 実装済み | A05 | モデレーション（非表示 / 復元 + 監査ログ） |
| ✅ 実装済み | A04 補足 | スペースカードに「設定」ボタン追加 |
| ✅ 実装済み | A01 | キャラ画像変更（DataURL保存） |
| ✅ 実装済み | A03 | Hossii 表情カスタム |
| ✅ 実装済み | A02 | スペース装飾（掲示板ウィジェット） |
