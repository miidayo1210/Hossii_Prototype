...
| 2 | 安全に特定できる旧 `authorId`（任意パラメータ、1件のみ） |
| 3 | ニックネーム一致（**同一スペース内で1件のみ**） |
| 4 | 0件 → 空状態 / 2件以上 → 曖昧エラー |

ニックネームが2件以上一致する場合は、別人のログを開かず次を表示する。
...
| 項目 | 状態 |
|---|---|
| データモデル `hossii_custom_config` | 実装済み（dev DB 適用済み） |
...
一般ユーザー向け表示名は **「ベースから作る」**（エディター内は「準備版」表記）。パーツ編集可能と誤解されないよう、ベース3種の選択のみであることを明記する。
