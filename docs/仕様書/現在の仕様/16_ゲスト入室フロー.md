...
> **分類:** `[Core]` ゲスト参加フロー・GuestEntryScreen
> **関連:** [02_匿名利用機能.md](./02_匿名利用機能.md) / [core-concepts.md](../../core/core-concepts.md) §2（Userの3種類）
> **共有URL参加導線の正本:** [123_URLからスペース参加UI.md](./docs/123_URLからスペース参加UI.md) — `GuestEntryScreen` / `ParticipantLoginScreen` / `participation_mode` / 再訪 UI の詳細は **123 を正**とする。本書と矛盾する場合は 123 を優先する。
...
> **123 との関係:** 現行の共有URL参加導線（参加方法選択・再訪確認・`participation_mode` 分岐など）は [123](./docs/123_URLからスペース参加UI.md) が正本。下記フロー図の「2回目以降スキップ」は旧整理であり、123 では再訪確認画面を挟む場合がある。
...
PC でカード名義・氏名などの保存済み提案が出ないよう、  
`nicknameInputAntiAutofillProps`（`src/core/utils/nicknameInputProps.ts`）を `<input>` に適用する。  
詳細は [37_スペースシェアURLニックネーム](./37_スペースシェアURLニックネーム.md) §問題1 を参照。

---

## 初回 QR アクセス時の表示（not-found 誤表示）

QR 読み取り直後に「スペースが見つかりません」が一瞬出ることはない仕様とする。

| タイミング | 表示 |
|------------|------|
| `fetchSpaceByUrl` 実行中 | エラー画面は出さない（`isFetchingSpaceFromSlug === true`） |
| 取得完了・スペースあり | `GuestEntryScreen` または `SpaceScreen` |
| 取得完了・スペースなし | 「スペースが見つかりません」 |

実装は `App.tsx` の `isFetchingSpaceFromSlug` と [03_スペースURL機能](./03_スペースURL機能.md) §初回アクセス時の not-found 誤表示防止 を参照。
