...
> **ステータス:** Phase 0–10B 実装済み（2026-06-27）。タブ追加 UX・デモ永続化・super_admin RLS 追従済み（2026-06-27）。10F/G/H は要件確定待ち
...
既存投稿については、初期移行時に `space_pane_id` を一括更新しない（方式 B）。
...
**Phase 10A（任意）:** migration により本番 DB の NULL 行を default pane id へ backfill 可能（冪等）。**Phase 10B** 以降、Supabase からの default Pane fetch は `space_pane_id = defaultPaneId` のみ。クライアントの `matchesPane` / `effectivePaneId` はデモ・レガシー INSERT 向けに `spacePaneId ?? defaultPaneId` を維持する。
...
* Pane固有設定の編集
* Pane固有URL・QRコードの取得
* 投稿の Pane 移動（`#comments` ログ一覧。Phase 10C）
...
**デモ（Supabase 未設定）:** DB 層は使えないため **A のみ**（`resolveDefaultPane` が synthetic pane を返すフォールバックは §19.2 に限定）。**追加 Pane の作成**は `demoSpacePanesStorage.ts`（localStorage、`hossii_demo_space_panes:{spaceId}`）に永続化し、`SpacePaneProvider` 読込時に synthetic default とマージする。本番 Supabase 接続時は DB が正本で localStorage の demo pane は使わない（§30.11）。
...
**Supabase fetch（Phase 10B 以降）**

デフォルトPaneでは、明示的な default pane id の投稿のみ取得する。

```text
space_id = currentSpaceId
AND space_pane_id = defaultPaneId
```
...
追加Paneの取得条件に `space_pane_id IS NULL` や `defaultPaneId` を含めてはならない。

**Phase 10A backfill 前の DB** では NULL 行が残る。backfill 適用後は上記条件で default Pane に含まれる。

**デモ / オフライン** では §11.1 の `effectivePaneId` により、`spacePaneId` 未設定の行も default Pane 表示対象とする（fetch ではなく store 内 filter）。
...
**スーパー管理者:** ✅ 実装済み。[20260629110000_space_panes_super_admin_rls.sql](./../../../supabase/migrations/20260629110000_space_panes_super_admin_rls.sql) で `app_metadata.role = 'super_admin'` の JWT に **全 `space_panes` の SELECT**（非表示 Pane 含む）を許可。コミュニティ管理者ポリシーと PERMISSIVE OR 合成。
...
**スーパー管理者:** ✅ 実装済み。[20260629110000_space_panes_super_admin_rls.sql](./../../../supabase/migrations/20260629110000_space_panes_super_admin_rls.sql) で INSERT / UPDATE を許可。`AuthContext` 上は `isSuperAdmin: true` かつ `isAdmin: true` のため UI（`＋` ボタン・設定画面タブ管理）はコミュニティ管理者と同様に表示される。**RLS は JWT の `super_admin` ロールが正本**（アプリの mock `isAdmin` だけでは Supabase INSERT は不可）。

**作成失敗時のエラー表示:** `createSpacePane` は Supabase エラーコードに応じた日本語メッセージを返す（例: `42501` → 管理者ログイン必要、`23505` → slug 重複）。
...
| 項目 | 延期先 | 理由 | 状態 |
|------|--------|------|------|
| コメント「すべてのタブ」フィルタ | Phase 9B | 現 Pane のみで本体価値は成立 | ✅ 実装済み |
| `#mylogs` Pane フィルタ | Phase 10E | 横断画面。初期スコープ外 | ✅ 実装済み |
| Pane 設定の Realtime 反映 | Phase 10D | 再読み込み・設定画面復帰で代替 | ✅ 実装済み |
| `archived_at` | Phase 10F | `is_visible` で十分 | 未着手 |
| 分析イベント §22 | — | 分析基盤なし | 対象外 |
| ドラッグ&ドロップ並び替え | Phase 6 以降 | 上下ボタンで可 | 対象外 |
| レガシー NULL backfill | Phase 10A | 方式 B で初期不要。任意整理 | ✅ 実装済み |
...
### Phase 5：タブバー — ✅ 実装済み（2026-06-27）

* SpacePaneBar作成
* 一般参加者の表示条件
* 管理者の表示条件
* 切り替えUI
* `＋` ボタン（タブ行末尾インライン、`SpacePaneCreateDialog`）
* 背景切り替え
* Paneごとの投稿表示
* **大画面表示（Fullscreen API）:** ダイアログは `document.fullscreenElement ?? document.body` へ portal（`useModalPortalRoot`）。スペースコンテナ外の `body` 直 portal では全画面時に非表示になる問題を回避
* **IME:** タブ名入力で Enter 1 回目（変換確定）は保存しない。`isComposing` / `keyCode 229` 除外後の Enter で保存
...
関連: `MoveHossiiPaneSelect.tsx`, `LogListBody.tsx`, `reconcileHossiiQueryKeys.ts`, `hossiisApi.ts`
...
* デフォルトPaneには `space_pane_id = defaultPaneId` の投稿が表示される（Supabase fetch）
* デモ環境では `effectivePaneId` により `spacePaneId` 未設定も default 扱い
* 追加Paneには defaultPaneId 以外の投稿のみ表示される
* Pane Aの投稿がPane Bに表示されない
* Realtime投稿が異なるPaneに混入しない
* 楽観的投稿が異なるPaneに混入しない
* ページング結果が異なるPaneに混入しない
* Pane切り替え中の古いfetch結果が混入しない
* 管理者による Pane 移動後、移動元 query から除去・移動先 query に追加される（Phase 10C、`reconcileHossiiQueryKeys`）
...
* 管理者は `#comments` から投稿を別 Pane へ移動できる（Pane 2 件以上、Phase 10C）
...
* 管理者のみ `#comments` に「タブへ」移動 UI（Pane 2 件以上）
* 移動後、active Pane 表示から除去され移動先 Pane に表示される
* Realtime 他端末でも `APPLY_REALTIME_PANE_UPDATE` で整合
* `#comments` all-panes モードでも query key 整合（`reconcileHossiiQueryKeys`）
* 他管理者の Pane 名称変更が Realtime で反映される（Phase 10D）
* active Pane 非表示化で default Pane へフォールバック（Phase 10D）
* `#mylogs`「このスペース」+ Pane 2 件以上で Pane フィルタ表示（Phase 10E）
* backfill migration 適用後、本番 NULL `space_pane_id` が 0 件であること
...
| 9 | Comments「すべてのタブ」 | **Phase 9B で実装済み** |
| 10 | Pane メタ Realtime | **Phase 10D で実装済み**（INSERT/UPDATE → `reloadPanesAndSyncActive`） |
...
| # | 項目 |
|---|------|
| 17 | スーパー管理者 RLS の `space_panes` 追従 | **✅ 20260629110000 migration** |
| 18 | `settings jsonb` の厳密 TS 型の最終形 |
| 19 | レガシー NULL backfill | **Phase 10A で migration 追加済み**（本番適用は運用判断） |
...
コメント一覧                → 現在Pane + 全Paneフィルタ（Phase 9B）。mylogs は Phase 10E
状態管理                    → URL + SpacePaneContext + SpacePaneRuntimeBridge（ref）
デフォルト方式              → default Pane レコード + 既存投稿 NULL 論理所属（方式 B）
                            → Phase 10A backfill / 10B fetch 整理は任意（§3.1）
...
| ログ一覧 | [44_ログ一覧](./44_ログ一覧画面.md) [83_私のログ切替](./83_ログ一覧の私のログ切替.md) | PaneFilterSegment・投稿 Pane 移動・mylogs Pane フィルタ |
| 設定画面 | [09_管理画面UI](./09_スペース管理画面UI仕様書.md) [92_管理スペース設定](./92_管理スペース設定.md) | タブ管理・Pane selector |
| 権限 | [10_ログインユーザー](./10_ログインユーザー管理設計.md) [27_User別](./27_User別.md) | `isAdmin` 流用。super_admin は `space_panes` RLS 別途（§18.2） |
...
**00_仕様概要.md** — §9 マイログに Pane フィルタ追記済み（Phase 10E）。
...
| Pane 作成失敗 | toast + リトライ。既存 Pane 一覧は維持。Supabase エラーはコード別日本語（§18.2） |
...
| イベント | 挙動 |
|----------|------|
| INSERT | `reloadPanesAndSyncActive` |
| UPDATE | `reloadPanesAndSyncActive`（名称変更・非表示化等。active が hidden なら default へ — §19.3） |
| DELETE | UI 非公開のため未対応 |

Space 単位で Realtime チャンネル 1 本。`space_panes` は `supabase_realtime` publication に追加（migration `20260629100000_phase10_backfill_and_pane_realtime.sql`）。

設定画面保存との競合は Last-write-wins（§30.10）。
...
### 30.17 タブ追加ダイアログ・環境別保存

| 項目 | 仕様 |
|------|------|
| コンポーネント | `SpacePaneCreateDialog.tsx`（スペース画面 `＋`・設定「タブ管理」共通） |
| 権限（UI） | `isAdmin`（コミュニティ管理者 **または** `isSuperAdmin`） |
| 権限（DB） | RLS: コミュニティ `admin_id = auth.uid()` **または** JWT `app_metadata.role = 'super_admin'`（§18.1, §18.2） |
| デモ（Supabase 未設定） | `createSpacePane` → `demoSpacePanesStorage`（localStorage）。`loadPanesForSpace` で synthetic default + 追加分をマージ |
| 本番（Supabase 設定済み） | `space_panes` へ INSERT。Realtime（Phase 10D）で他セッションにも反映。migration 適用・管理者 JWT 必須 |
| 全画面表示 | portal 先は `useModalPortalRoot`（Fullscreen 要素内）。仕様 [71](./71_大画面表示.md) のスペースルートと音声パネルと同型 |
| IME Enter | 変換確定 Enter は無視。確定後の Enter で「作成」と同等 |
| 作成成功 | `reloadPanesAndSyncActive` → 新 Pane を active に切替 → success toast |

**デプロイ時の注意:** 環境変数で Supabase URL / anon key が設定されていれば **デモ localStorage ではなく DB が正本**。`20260629000000_add_space_panes.sql` および `20260629110000_space_panes_super_admin_rls.sql`（super_admin 利用時）を適用すること。
