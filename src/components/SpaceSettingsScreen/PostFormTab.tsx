...
export const PostFormTab = ({ settings, onUpdate, onDirtyChange }: Props) => {
  const { draft, setDraft, isDirty, discard, commitSaved } = useScreenDraft(settings);
  ...
  const handleSave = async () => {
    ...
    try {
      onUpdate(draft);
      saveSpaceSettings(draft);
      await upsertSpaceSettings(draft);
      commitSaved(draft);
      ...
      const message =
        err instanceof PostFieldsColumnMissingError
          ? 'Supabase に post_fields 列がありません。マイグレーションを適用してください。'
          : '保存に失敗しました';
