...
function storageKeyForSpace(spaceId: string): string | null {
  ...
  return scopedStorageKey(`hossii.spaceGuideSeen.${trimmed}`);
}
...
export function hasSeenSpaceGuide(spaceId: string): boolean {
...
export function markSpaceGuideSeen(spaceId: string): void {
  const key = storageKeyForSpace(spaceId);
  if (!key) return;
  try {
...
export function spaceGuideStorageKey(spaceId: string): string | null {
