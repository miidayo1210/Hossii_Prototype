-- Make submit_challenge_comment_response idempotent under concurrent first submits.
-- Replace SELECT→INSERT/UPDATE response path with atomic UPSERT on UNIQUE (item_id, user_id).
-- Keep completion/reward one-shot semantics; re-link completion.response_id after answer delete.
...
BEGIN
  ...
  -- user_id is always auth.uid(); guard trigger also pins user_id/item_id.
  -- updated_at is owned by challenge_responses_set_updated_at trigger (do not set here).
...
COMMENT ON FUNCTION public.submit_challenge_comment_response(uuid, text, text) IS
  'P5+: atomic comment response UPSERT + completion + one-time Hossii reward. Concurrent-safe. No client user_id/hossii_key.';
